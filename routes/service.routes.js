...
router.post("/create", upload.array("images", 5), async (req, res) => {
  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      imageUrls = await Promise.all(
        req.files.map(async (file) => {
          const fileName = `${serviceProviderId}/${serviceId}/${Date.now()}-${
            file.originalname
          }`;
    ...
    await newService.update(
      ...
      { where: { serviceId: newService.dataValues.serviceId }, transaction }
    );
...
router.patch("/:id/status", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Service status updated successfully", service });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
router.get("/images/:serviceId", async (req, res) => {
  try {
    ...
    const imageUrls = service.demoPics ? JSON.parse(service.demoPics) : [];
