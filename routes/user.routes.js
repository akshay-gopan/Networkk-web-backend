...
router.post("/favorites", authenticateToken, async (req, res) => {
  const { id } = req.body; // Change to serviceId for clarity
  ...
  try {
    ...
    // Retrieve and parse existing favorites safely
    let favorites = [];
    if (user.favorites) {
      try {
    ...
    if (!favorites.includes(id)) {
      favorites.push(id); // Add new serviceId while keeping existing ones

      await user.update({
        favorites: JSON.stringify(favorites), // Ensure it's stored as JSON
      });
